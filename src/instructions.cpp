...
// -- LOAD --
void CPU::LD_B_U8(){ LD(&registers.B, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::LD_C_U8(){ LD(&registers.C, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::LD_D_U8(){ LD(&registers.D, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::LD_E_U8(){ LD(&registers.E, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::LD_H_U8(){ LD(&registers.H, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::LD_L_U8(){ LD(&registers.L, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::LD_A_U8(){ LD(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }

void CPU::LD_SP_U16(){ LD16(&registers.SP, memory.Read16Mem(registers.PC)); } 
void CPU::LD_SP_HL(){ LD16(&registers.SP, registers.Pair(registers.H, registers.L)); }

void CPU::LD_A_ADDR_BC(){ LD(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.B, registers.C))); }
void CPU::LD_A_ADDR_DE(){ LD(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.D, registers.E))); }
void CPU::LD_A_ADDR_HLp(){ 
    LD(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L)));     
...
void CPU::LD_A_ADDR_U16(){ LD(&registers.A, memory.GetHostAddressMem(memory.Read16Mem(registers.PC))); }
...
void CPU::LD_HL_SPpI8(){ 
    uint16_t val = registers.SP + (int8_t)memory.Read8Mem(registers.PC); 
...
void CPU::LD_B_ADDR_HL(){ LD(&registers.B, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::LD_C_ADDR_HL(){ LD(&registers.C, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::LD_D_ADDR_HL(){ LD(&registers.D, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::LD_E_ADDR_HL(){ LD(&registers.E, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::LD_H_ADDR_HL(){ LD(&registers.H, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::LD_L_ADDR_HL(){ LD(&registers.L, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::LD_A_ADDR_HL(){ LD(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }

void CPU::LD_ADDR_HL_B(){ memory.Write8Mem(registers.Pair(registers.H, registers.L), registers.B); }
void CPU::LD_ADDR_HL_C(){ memory.Write8Mem(registers.Pair(registers.H, registers.L), registers.C); }
void CPU::LD_ADDR_HL_D(){ memory.Write8Mem(registers.Pair(registers.H, registers.L), registers.D); }
void CPU::LD_ADDR_HL_E(){ memory.Write8Mem(registers.Pair(registers.H, registers.L), registers.E); }
void CPU::LD_ADDR_HL_H(){ memory.Write8Mem(registers.Pair(registers.H, registers.L), registers.H); }
void CPU::LD_ADDR_HL_L(){ memory.Write8Mem(registers.Pair(registers.H, registers.L), registers.L); }
void CPU::LD_ADDR_HL_A(){ memory.Write8Mem(registers.Pair(registers.H, registers.L), registers.A); }
void CPU::LD_ADDR_BC_A(){ memory.Write8Mem(registers.Pair(registers.B, registers.C), registers.A); }
void CPU::LD_ADDR_DE_A(){ memory.Write8Mem(registers.Pair(registers.D, registers.E), registers.A); }
void CPU::LD_ADDR_HLp_A(){ 
    memory.Write8Mem(registers.Pair(registers.H, registers.L), registers.A);
...
void CPU::LD_ADDR_U16_A(){ memory.Write8Mem(memory.Read16Mem(registers.PC), registers.A); }
void CPU::LD_ADDR_U16_SP(){ memory.Write8Mem(memory.Read16Mem(registers.PC), registers.SP); }
void CPU::LD_ADDR_HL_U8(){ memory.Write8Mem(registers.Pair(registers.H, registers.L), memory.Read8Mem(registers.PC)); }
...
void CPU::LD_OFF_U8_A() { memory.Write8Mem(0xFF00 + memory.Read8Mem(registers.PC), registers.A); }
void CPU::LD_A_OFF_U8() { LD(&registers.A, memory.GetHostAddressMem(0xFF00 + memory.Read8Mem(registers.PC))); }
void CPU::LD_A_OFF_C() { LD(&registers.A, memory.GetHostAddressMem(0xFF00 + registers.C)); }
void CPU::LD_OFF_C_A() { memory.Write8Mem(0xFF00 + registers.C, registers.A); }
...
void CPU::INC_ADDR_HL(){ INC(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::DEC_ADDR_HL(){ DEC(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::ADD_A_U8(){ ADD(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::ADD_A_ADDR_HL(){ ADD(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::ADC_A_U8(){ ADC(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::ADC_A_ADDR_HL(){ ADC(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::ADD_SP_I8(){ 
    uint32_t res = registers.SP + (int8_t)memory.Read8Mem(registers.PC);   
    flags.Z ^= flags.Z; 
    flags.N ^= flags.N;
    flags.C = ((res & 0x100000000) != 0);
    flags.H = (((registers.SP & 0xfff) + ((int8_t)memory.Read8Mem(registers.PC) & 0xfff)) & 0x1000) != 0;
...
void CPU::SUB_A_U8(){ SUB(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::SUB_A_ADDR_HL(){ SUB(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::SBC_A_U8(){ SBC(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::SBC_A_ADDR_HL(){ SBC(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::AND_A_U8(){ AND(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::AND_A_ADDR_HL(){ AND(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::XOR_A_U8(){ XOR(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::XOR_A_ADDR_HL(){ XOR(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::OR_A_U8(){ OR(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::OR_A_ADDR_HL(){ OR(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::CP_A_U8(){ CP(&registers.A, memory.GetHostAddressMem(registers.PC + 1)); }
void CPU::CP_A_ADDR_HL(){ CP(&registers.A, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }


// -- BRANCH --
void CPU::JR_I8(){ JR((int8_t)memory.Read8Mem(registers.PC)); }
void CPU::JR_C_I8(){ if(flags.C) JR((int8_t)memory.Read8Mem(registers.PC)); }
void CPU::JR_Z_I8(){ if(flags.Z) JR((int8_t)memory.Read8Mem(registers.PC)); }
void CPU::JR_NC_I8(){ if(!flags.C) JR((int8_t)memory.Read8Mem(registers.PC)); }
void CPU::JR_NZ_I8(){ if(!flags.Z) JR((int8_t)memory.Read8Mem(registers.PC)); }

void CPU::JP_U16(){ JP(memory.Read16Mem(registers.PC)); }
void CPU::JP_Z_U16(){ if(flags.Z) JP(memory.Read16Mem(registers.PC)); }
void CPU::JP_C_U16(){ if(flags.C) JP(memory.Read16Mem(registers.PC)); }
void CPU::JP_NZ_U16(){ if(!flags.Z) JP(memory.Read16Mem(registers.PC)); }
void CPU::JP_NC_U16(){ if(!flags.C) JP(memory.Read16Mem(registers.PC)); }
...
void CPU::CALL_U16(){ CALL(memory.Read16Mem(registers.PC)); }
void CPU::CALL_Z_U16(){ if(flags.Z) CALL(memory.Read16Mem(registers.PC)); }
void CPU::CALL_C_U16(){ if(flags.C) CALL(memory.Read16Mem(registers.PC)); }
void CPU::CALL_NZ_U16(){ if(!flags.Z) CALL(memory.Read16Mem(registers.PC)); }
void CPU::CALL_NC_U16(){ if(!flags.C) CALL(memory.Read16Mem(registers.PC)); }
...
void CPU::RLC_ADDR_HL(){ RLC(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::RRC_ADDR_HL(){ RRC(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::RL_ADDR_HL(){ RL(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::RR_ADDR_HL(){ RR(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::SLA_ADDR_HL(){ SLA(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::SRA_ADDR_HL(){ SRA(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::SRL_ADDR_HL(){ SRL(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::SWAP_ADDR_HL(){ SWAP(memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::BIT_0_ADDR_HL(){ BIT(0, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::BIT_1_ADDR_HL(){ BIT(1, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::BIT_2_ADDR_HL(){ BIT(2, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::BIT_3_ADDR_HL(){ BIT(3, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::BIT_4_ADDR_HL(){ BIT(4, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::BIT_5_ADDR_HL(){ BIT(5, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::BIT_6_ADDR_HL(){ BIT(6, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::BIT_7_ADDR_HL(){ BIT(7, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::RES_0_ADDR_HL(){ RES(0, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::RES_1_ADDR_HL(){ RES(1, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::RES_2_ADDR_HL(){ RES(2, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::RES_3_ADDR_HL(){ RES(3, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::RES_4_ADDR_HL(){ RES(4, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::RES_5_ADDR_HL(){ RES(5, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::RES_6_ADDR_HL(){ RES(6, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::RES_7_ADDR_HL(){ RES(7, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
...
void CPU::SET_0_ADDR_HL(){ SET(0, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::SET_1_ADDR_HL(){ SET(1, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::SET_2_ADDR_HL(){ SET(2, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::SET_3_ADDR_HL(){ SET(3, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::SET_4_ADDR_HL(){ SET(4, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::SET_5_ADDR_HL(){ SET(5, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::SET_6_ADDR_HL(){ SET(6, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
void CPU::SET_7_ADDR_HL(){ SET(7, memory.GetHostAddressMem(registers.Pair(registers.H, registers.L))); }
